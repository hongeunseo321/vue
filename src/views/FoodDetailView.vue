...
export default({
    ...
    mounted() {
        /*
            생명주기 함수 = 자동 호출
                **created: Vue 객체 생성 완료 => params를 받는 경우
                    => this.$route.parama.키 명
                        params:{fno:1, id:'aaa', name:'bbb'}
                **mounted: 시작과 동시에 서버로부터 값을 읽어오는 경우
                updated: 사용자가 데이터 값을 변경한 경우
                    <input type=text>: 검색
                unmounted: 다른 화면으로 이동 시에 vue 객체를 메모리 해제
        ...
        */
        axios.get('http://localhost:8080/web/food/detail_vue.do', {
            ...
            if (window.kakao && window.kakao.maps) {
                ...
                this.addScript(this.food_detail.address,this.food_detail.name)
            }
        })
    },
    methods: {
        addScript() {
            ...
            script.src = "http://dapi.kakao.com/v2/maps/sdk.js?autoload=false&appkey=365409a3cc6e15c3dc1650ef18ae732f&libraries=services"
